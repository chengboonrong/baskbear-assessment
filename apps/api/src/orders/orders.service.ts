      ...
      const order = await this.prisma.$transaction(
        ...
        `;
          const cart = await tx.cart.findUnique({
            where: {
              userId_countryId: { userId, countryId: country.countryId },
            },
            include: {
              items: {
                include: {
          ...
          const itemRows = cart.items.map((it) => {
            ...
            if (!current || !current.isAvailable) {
              throw new BadRequestException(
                `ITEM_NO_LONGER_AVAILABLE:${it.menuItemId}`,
              );
            }
            const customs =
              (it.customisationsJson as unknown as Array<{
                deltaMinor: number;
              }>) ?? [];
            ...
            return {
              ...
              sku: it.menuItem.sku,
              nameSnapshot:
                it.menuItem.translations[0]?.name ?? it.menuItem.sku,
              quantity: it.quantity,
              unitPriceMinor: unitPrice,
              customisationsSnapshotJson:
                it.customisationsJson as Prisma.InputJsonValue,
          ...
          if (input.voucherCode) {
            ...
            if (!v || !v.isActive)
              throw new BadRequestException('VOUCHER_INVALID');
            const now = new Date();
            if (now < v.startsAt || now > v.endsAt)
            ...
            if (subtotal < v.minSpendMinor)
              throw new BadRequestException('VOUCHER_MIN_SPEND_NOT_MET');

            const userUses = await tx.voucherRedemption.count({
              where: { voucherId: v.id, userId },
            });
            if (userUses >= v.perUserLimit)
              throw new BadRequestException('VOUCHER_USER_LIMIT');
            if (v.totalLimit !== null) {
              const total = await tx.voucherRedemption.count({
